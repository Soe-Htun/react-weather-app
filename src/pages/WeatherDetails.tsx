  ...
  useEffect(() => {
    ...
    const loadWeather = async () => {
      try {
        ...
        const forecastList = await fetchWeatherForecast(
          data.coord.lat,
          data.coord.lon
        );
        ...
        navigate("/");
        toast.error(
          "Unable to load weather data. Please check the city name or try again later."
        );
      }
    };
      ...
      <div className="bg-primary text-white p-2">
        <div className="flex justify-between items-center">
          ...
          <button onClick={handleToggleSave}>
            {isSaved ? (
              <BasicIcons name="trash" className="w-6 h-6" />
            ) : (
